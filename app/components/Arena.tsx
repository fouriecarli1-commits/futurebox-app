...
import {
  ...
  Music, Video, Code2, Lightbulb, Gift, Scale, RefreshCw,
} from 'lucide-react';
import {
  COMPETITIONS, CATEGORY_LABELS, type Competition, type CompetitionCategory,
} from '../data/studio';
import { generateCompetition, formatFee } from '../lib/matching';
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="text-[10px] font-mono text-amber-300 bg-amber-500/10 border border-amber-500/30 rounded-lg px-2.5 py-1.5">
          ...
          return (
            ...
            >
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center space-x-2 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] font-mono text-zinc-500">{CATEGORY_LABELS[c.category]}</p>
                    </div>
                  </div>
                  <span className="text-[10px] font-mono text-emerald-300 bg-emerald-500/10 border border-emerald-500/30 rounded-lg px-2 py-1 flex-shrink-0">
                    {formatFee(c.entryFeeCents)}
                  </span>
                </div>

                <p className="text-[11px] text-zinc-400 leading-relaxed">{c.brief}</p>

                <div className="p-2.5 rounded-xl bg-black/40 border border-zinc-800 space-y-1">
                  <p className="text-[10px] text-amber-300 font-bold flex items-center space-x-1.5">
                    <Crown className="w-3 h-3" />
                    <span>{c.prize}</span>
                  </p>
                  <p className="text-[10px] text-zinc-500">Runner-up: {c.runnerUp}</p>
                </div>
              ...
              <button
                type="button"
                onClick={() => openEntry(c)}
                className="w-full py-2.5 rounded-xl bg-gradient-to-r from-amber-500 to-orange-400 text-black font-bold text-[11px] uppercase tracking-wider hover:opacity-90 transition-all"
              >
                Enter — {formatFee(c.entryFeeCents)} or free route
              </button>
            </div>
          );
        })}
      ...
      <div className="p-4 rounded-2xl bg-black/40 border border-zinc-800 space-y-3">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          ...
          <button
            ...
            className="px-2.5 py-1 rounded-lg text-[11px] font-mono bg-zinc-900 border border-zinc-700 text-zinc-300 hover:border-cyan-500 hover:text-cyan-300 flex items-center space-x-1.5"
          >
        ...
        <div className="p-3 rounded-xl bg-zinc-950 border border-zinc-800 space-y-1.5">
          ...
          <p className="text-[11px] text-zinc-400 leading-relaxed">{generated.brief}</p>
      ...
      {selected && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="w-full max-w-2xl bg-[#0b0d13] border border-zinc-800 rounded-2xl p-5 space-y-4 my-8">
            <div className="flex items-start justify-between gap-3">
              <div>
            ...
            <div className="p-3.5 rounded-xl bg-zinc-950 border border-zinc-800 space-y-2">
              <p className="text-[11px] font-bold text-white flex items-center space-x-1.5">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                <p className="text-[11px] font-bold">Paid entry — {formatFee(selected.entryFeeCents)}</p>
                <p className="text-[10px] text-zinc-500">Covers judging and hosting. Nothing else.</p>
              </button>
              <button
                type="button"
              ...
              >
                <p className="text-[11px] font-bold flex items-center space-x-1"><Gift className="w-3 h-3" /><span>Free entry</span></p>
                <p className="text-[10px] text-zinc-500">Same prize, same judging, no payment.</p>
              </button>
            </div>

            <form onSubmit={submitEntry} className="space-y-3">
              <div className="space-y-1.5">
                <label className="text-[11px] font-mono text-zinc-400">Link to your entry</label>
                <input
                  value={entryLink}
                  ...
                  className="w-full bg-black/60 border border-zinc-800 rounded-lg px-3 py-2 text-[11px] text-white focus:outline-none focus:border-amber-500"
                />
              </div>
              ...
              <div className="space-y-2">
                {[
                  ...
                  <label key={row.label} className="flex items-start space-x-2.5 cursor-pointer">
                    ...
                    />
                    <span className="text-[10px] text-zinc-300 leading-relaxed">{row.label}</span>
                  </label>
                ))}
              </div>

              {entryStatus === 'incomplete' && (
                <p className="text-[10px] text-amber-300 flex items-center space-x-1.5">
              ...
              {entryStatus === 'paid_pending' && (
                <p className="text-[10px] text-cyan-300 bg-cyan-950/30 border border-cyan-500/30 rounded-xl p-2.5 leading-relaxed">
              ...
              {entryStatus === 'free_pending' && (
                <p className="text-[10px] text-emerald-300 bg-emerald-950/30 border border-emerald-500/30 rounded-xl p-2.5 leading-relaxed flex items-start space-x-2">
              ...
              <button
                type="submit"
                className="w-full py-3 rounded-xl bg-gradient-to-r from-amber-500 to-orange-400 text-black font-bold text-[11px] uppercase tracking-wider hover:opacity-90 transition-all"
              >
                {entryRoute === 'paid' ? `Pay ${formatFee(selected.entryFeeCents)} and enter` : 'Enter free'}
              </button>
